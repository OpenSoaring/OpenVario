...
function select_image(){
    ...
    while read -r line; do # process file by file
        ...
        files_nice+=($i "$temp1 $temp2 $temp3") # selection index + name
    done < <( ls -1 $images )
    ...
    if [ ! -e "$IMAGEFILE" ]; then
        ...
        echo "selected image file:    $(basename $IMAGEFILE)"
    fi
...
function start_upgrade(){
    ...
    #August: =============================================
    echo "Das ist eine Test-Unterbrechung!!!"
    read INPUT
    exit
    #================================
...
if [ -f "${IMAGEFILE}" ]; then
    echo "Start..."
    ### #August: =============================================
    ### echo "Interrupt (after select)!!!"
    ### # read INPUT
    ### # -rsp $'Press enter to continue...(start_upgrade)\n'
    ### exit
    ### #================================
    ...
    rsync -auv --progress $MOUNT_DIR1/* $BACKUP_DIR/part1/
    #      --exclude ...
    ...
    rsync -auv --progress $MOUNT_DIR2/home/root/.xcsoar/* $BACKUP_DIR/part2/xcsoar/ \
